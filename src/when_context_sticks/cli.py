...
def paper_main() -> None:
    ...
    if args.command == "arch-sweep":
        ...
        write_arch_sweep_summary(
            manifest,
            PAPER_ROOT / "table_1_architecture_sweep.csv",
        )
        table_2_csv = write_hyperparameter_table(
            PAPER_ROOT / "table_2_hyperparameters.csv"
        )
        print(table_2_csv.relative_to(project_root()))
        return
    ...
    if args.command == "all":
        arch_manifest = train_config_set(
            ARCH_SWEEP_CONFIGS,
            output_root=project_root() / "artifacts" / "runs" / "paper" / "arch_sweep",
            manifest_path=PAPER_ROOT / "arch_sweep" / "run_manifest.json",
            seed=args.seed,
        ...
        write_hyperparameter_table(PAPER_ROOT / "table_2_hyperparameters.csv")
        train_config_set(
            SWITCH_STUDY_CONFIGS,
            output_root=(
                project_root() / "artifacts" / "runs" / "paper" / "switch_study"
